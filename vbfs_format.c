...
{
	if (sizeof(struct vbfs_superblock) != VBFS_SUPER_SIZE) {
		fprintf(stderr, "sizeof(struct vbfs_superblock) != %d\n", VBFS_SUPER_SIZE);
		return -EINVAL;
	}

	if (sizeof(struct vbfs_inode) != INODE_SIZE) {
		fprintf(stderr, "sizeof(struct vbfs_inode) != %d\n", INODE_SIZE);
		return -EINVAL;
	}
